...
- Instagram collection attempts to traverse to the first slide and then walk
  ...
  Backward and forward traversal each stop after at most 30 transitions, a
  repeated state, or a transition with no change; restoration to the original
  slide is separately bounded. Collection may be incomplete when Instagram's
  DOM or lazy transition does not expose a slide. Images and videos remain
  distinct candidates.
